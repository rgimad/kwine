...
/* --------------------------
Check wheter PE is valid or not
in:     raw      - pointer to the pe image in memory (not loaded to ImageBase yet, only readed from file)
        raw_size - size of raw image
        is_exec  - non-zero if EXE, zero if DLL
out:
        0        - if given PE is invalid
        1        - if it is valid
*/
...
/* --------------------------
Fast copying block [src, src + len) to address dst
*/
...
/* --------------------------
Loads given PE raw image to free place and makes relocations
in:     raw - pointer to raw data
out:    void* pointer to base of loaded image 
*/
void* create_image(void *raw)
{
    PIMAGE_DOS_HEADER     dos;
...
/* --------------------------
Fill in the import table of given PE image
in:     img_base - base of PE image
        imp      - its import table
out:
        0        - fail
        1        - success
how works:
        Goes through import table of given PE and for every library in it does the following:
            - loads the library by calling load_library   (see line: "api = load_library(libname);")
            - writes addresses of functions needed functions from loaded library to the import table of given PE (imp)
*/
int link_image(void *img_base, PIMAGE_IMPORT_DESCRIPTOR imp)
{
...
/*
Returns RVA address of entry point of given PE
in:     raw - pointer to raw data
out:    void* RVA address of entry point 
*/
...
/*
Searches a procedure in export table by name
in:    handle - pointer to loaded dll's module_t structure
       proc_name - name of procedure
out:   void* address of procedure
       NULL if procedure not found
*/
...
/*
Function for internal use (used by load_library )
in:    path - path to dll
out:   void* pointer to base of loaded image 
       or NULL if failed
*/
...
/* --------------------------
Loads the library by it's name(or path?)
in:     name - name (or path?) of the library
out:    pointer to the module_t structure of loaded library
        or NULL if failed
*/
...
//            printf("%s\n", path);
    ...
    if(nt->OptionalHeader.DataDirectory[1].Size) // if import directory is not empty
    {
        PIMAGE_IMPORT_DESCRIPTOR imp;
...
/*
Enumerate all loaded libraries, that are stored in list of module_t structures
in:    callback - pointer to function, which called at each iteration of enumeration
       user_data- some arg ?
out:   nothing
*/
